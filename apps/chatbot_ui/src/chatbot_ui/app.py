...
def submit_feedback(feedback_type=None, feedback_text=""):
    """
    Sumbit feedback to the API endpoint.
    """
...
for idx, message in enumerate(st.session_state.messages):
    ...
    # Add feedback buttons for the latest assistant message (excluding the initial greeting)
    is_latest_assistant = (
    ...
    if is_latest_assistant:
        ...
        if st.session_state.show_feedback_box:
            ...
            col_send, col_spacer, col_close = st.columns([1.5, 5, 1])

            with col_send:
                if st.button("Send Additional Details", key=f"send_additional_{len(st.session_state.messages)}"):
                    if feedback_text.strip():  # Only submit if there's text
